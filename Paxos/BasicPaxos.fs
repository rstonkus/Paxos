...
// timeout
// force reset (in case of too many acceptors failing. We can get stuck in an in-between state)
...
//      else ignore //Different session (old, since it is a response) => Ignore. 
    let retry ((_,k,_) as cr:ClientRequest) n n' proposedN = 
      if (n = n')
      then if (proposedN > n)
           then let retryN = proposedN + 1
                in (PPrepareSent (retryN,cr,[]), Some (BroadcastAcceptors, MPrepare (retryN,k)))
           else ignore
      else ignore
    match s with
    ...
    | PPrepareSent (n, cr, promises) ->
      ...
      | MPromise (n', v') -> 
        ...
        else ignore //a promise can never be newer, since it is a response. So it must be an old one
      | MAccepted _ -> ignore //Old => Ignore
      | MNackPrepare (n',newN) -> retry cr n n' newN
    | PAcceptSent (n, cr, accepts) ->
      ...
      | MAccepted (n', _, _, v) ->
        ...
        //even if this is a newer one from another session, we are forced to ignore it while collecting our responses. Otherwise we will risk losing a db update
        //this is probably the reason we need a force reset (in case less than quorum accepter are available)
  ...
  let acceptorReceiveFromProposer (AReady (n,store):AState) (m:PMsg) (sender:Sender) : (AState * (Destination * AMsg) option) =
    ...
    | MAccept (n',cs, k',v') ->
      ...
      then let store' = Map.add k' (n',v') store
           in (AReady (n', store'), Some (Broadcast, MAccepted (n, cs, k', v'))) //TODO maybe dont send to other proposers
      else (AReady (n, store), Some (Proposer sender, MNackPrepare (n',n)))
